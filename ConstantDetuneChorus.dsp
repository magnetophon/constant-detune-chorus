...
start = ba.time<1; //the initial trigger for the SH
//SampleRate = 44100;
...
slowFreq	= chorusGroup(vslider("[0]slow freq",	0.57, 0, 5, 0.01):si.smooth(0.999));
slowDepth	= chorusGroup(vslider("[1]slow depth",	4, 0, 5, 0.01):pow(2):si.smooth(0.999));
fastFreq	= chorusGroup(vslider("[2]fast freq",	1, 0, 10, 0.01):si.smooth(0.999));
fastDepth	= chorusGroup(vslider("[3]fast depth",	4, 0, 10, 0.01):pow(2):si.smooth(0.999));
feedback	= chorusGroup(vslider("[4]feedback",	0, -1, 1, 0.01)*0.8:si.smooth(0.999));
